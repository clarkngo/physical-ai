...
} from '../components/AviationExplorer';
import {
  DEFAULT_ATTACK_SCENARIO_ID,
  getInitialIncidentSteps,
  getScenarioById,
} from '../components/AviationExplorer/attackScenario.js';
import AviationSectionNav from '../components/AviationSectionNav';

export default function AviationExplorerPage() {
    ...
    <div className="flex flex-col gap-5">
      ...
      <header className="glass-card rounded-2xl px-5 py-4 md:px-6">
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-whiteHull md:text-3xl">
          Aviation CPS Explorer
        </h1>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-300">
          Explore airport ramp assets in 3D — avionics buses, surveillance feeds, and maintenance
          dataload paths. Toggle Attack mode to run incident response playbooks.
        </p>
      </header>

      <div
        ref={moduleRef}
        className="aviation-explorer-module h-[min(88vh,960px)] min-h-[640px] w-full rounded-2xl border border-white/10"
      >
        <AviationScene
