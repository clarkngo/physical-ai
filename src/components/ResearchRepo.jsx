...
const SOURCE_ACCENT = {
  'MIT Sea Grant (Open Access)': 'text-cyan-400 bg-cyan-400/10 border-cyan-400/25',
  'AUV Lab Dataset':             'text-blue-400 bg-blue-400/10 border-blue-400/25',
  'Maritime AI Consortium':      'text-indigo-400 bg-indigo-400/10 border-indigo-400/25',
};

// ── Tiny data-stream decoration ───────────────────────────────────────────
function DataStream() {
  return (
...
export default function ResearchRepo() {
  ...
  const activePaper = researchPapers.find((p) => p.id === activePaperId);
  ...
  return (
    <section className="space-y-5 pb-10">
      ...
      >
        ...
        <span className="mb-1 text-xs font-semibold uppercase tracking-widest text-pacificCyan">
          ...
          return (
            <motion.button
              ...
              className={`relative rounded-2xl border p-5 text-left transition-colors duration-200 ${
                isActive
                  ? 'border-pacificCyan bg-deepSea/80 shadow-[0_0_24px_rgba(0,163,224,0.15)]'
                  : 'border-slate-700 bg-slate-900/70 hover:border-pacificCyan/50'
              }`}
            >
              {/* Active indicator */}
              {isActive && (
                <motion.span
                  ...
                  className="absolute left-0 top-3 h-8 w-0.5 rounded-r-full bg-pacificCyan"
                />
              )}
              ...
              {isActive && (
                <motion.div
                  ...
                  className="mt-3 h-0.5 rounded-full bg-pacificCyan/40"
                />
              )}
            </motion.button>
          );
