...
import { createPortAmbientAudio } from './portAmbientAudio.js';
...
export default function MaritimeScene({
  selectedId,
  ...
  useEffect(() => {
    ...
    const portAudio = createPortAmbientAudio();
    ...
    function onClick(event) {
      ...
      const ctx = portAudio.getContext();
      if (id) {
    ...
    const animate = () => {
      ...
      lights.harborGlow.intensity = THREE.MathUtils.lerp(0.65, 0.3, attackBlend);
    ...
    window.addEventListener('maritime-explorer-resize', onExplorerResize);
    ...
    return () => {
      ...
      window.removeEventListener('maritime-explorer-resize', onExplorerResize);
  ...
  return (
    <div className="maritime-viewport-layer overflow-hidden bg-[#0d1117]">
      <div ref={mountRef} className="h-full w-full" aria-label="Interactive 3D port environment" />
