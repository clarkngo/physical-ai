    ...
    <AnimatePresence>
      {concept ? (
        <motion.aside
          ...
          className={`aviation-hud aviation-hud-interactive flex max-h-[min(70vh,520px)] w-full flex-col overflow-hidden rounded-xl ${
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="aviation-hud hidden max-w-xs rounded-xl px-4 py-4 md:block"
        >
          <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-pacificCyan">
            Concept Panel
          </p>
          <h2 className="mt-2 text-sm font-semibold text-whiteHull">Select a ramp asset</h2>
