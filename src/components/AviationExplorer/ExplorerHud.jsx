...
const OBJECT_HINTS = Object.values(AVIATION_CONCEPTS).map(({ id, label, title }) => ({
  id,
  label,
  title,
}));
...
export default function ExplorerHud({
  ...
  return (
    <div className="aviation-hud-root">
      <div className="aviation-hud-topbar">
        <div className="aviation-hud pointer-events-none max-w-md rounded-xl px-4 py-3">
        ...
        <div className="aviation-hud-interactive flex flex-wrap items-center gap-2">
          <div className="aviation-hud rounded-xl p-1">
          ...
          {selectedId && !isAttack && (
            <button
              ...
              className="aviation-hud rounded-xl px-3 py-1.5 text-xs text-slate-200 transition hover:bg-white/10"
            >
              Clear
            </button>
          )}
      ...
      {!isAttack && conceptPanel && (
        <div className="aviation-concept-float">{conceptPanel}</div>
      )}

      {isAttack && (
        <div className="aviation-incident-panel">
          <IncidentDashboard
      ...
      <div
        className={`aviation-hud-dock-wrap ${bottomDockMinimized ? 'aviation-hud-dock-wrap--minimized' : ''}`}
      >
        <button
          type="button"
          onClick={onToggleBottomDock}
          className="aviation-hud-dock-toggle aviation-hud-interactive aviation-hud flex items-center gap-1.5 rounded-t-lg px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400 transition hover:text-slate-200"
        ...
        <div className="aviation-hud-dock aviation-hud">
          {isAttack && (
            <div className="border-b border-white/10">
              ...
              <div className="aviation-hud-interactive grid gap-1 p-2 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {['straddle_carrier', 'power_hub'].map((id) => {
                  const concept = AVIATION_CONCEPTS[id];
          ...
          <div className="aviation-hud-interactive grid gap-1 p-2 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="aviation-hud-interactive grid gap-1 p-2 sm:grid-cols-2">
