...
import adsbSignals from '../data/adsbSignals.json';

const levels = [
  {
    id: 1,
    title: 'Level 1: The Connected Airport',
    icon: '🛫',
    text: 'Explore how aircraft, ATC, and ramp robots share one cyber-physical fabric — from gate power to surveillance feeds.',
    accent: 'from-sky-500/20 to-slate-900/40',
  },
  {
    id: 2,
    title: 'Level 2: Avionics Digital Twins',
    icon: '✈️',
    text: 'A simplified FMS twin shows how waypoint updates and approach clearances propagate through navigation integrity checks.',
    accent: 'from-blue-500/20 to-slate-900/40',
  },
  {
    id: 3,
    title: 'Level 3: Surveillance Sensing',
    icon: '📡',
    text: 'Watch ADS-B replies turn into a range plot — the same idea as a radar scope, built with CSS + SVG animation.',
    accent: 'from-cyan-500/20 to-slate-900/40',
  },
];

function AdsbRangePlot() {
  const W = 560;
  const H = 200;
  const padX = 50;
  const stepX = (W - padX * 2) / (adsbSignals.length - 1);

  const points = adsbSignals.map((s, i) => ({
    x: padX + i * stepX,
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-sky-500/20 bg-slate-900/70 p-3">
      <p className="mb-2 text-xs font-medium uppercase tracking-widest text-sky-400">
        Live ADS-B Feed — KSEA Approach
      </p>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-44 w-full">
        ...
        {points.map((p, i) => (
          <motion.circle
            ...
            cx={p.x}
            cy={p.y}
            fill={p.strength > 0.6 ? 'rgba(56,189,248,0.85)' : 'rgba(56,189,248,0.45)'}
...
export default function AviationLearningPathway() {
  return (
    <section id="learning" className="space-y-6">
      ...
      >
        Learning Pathway
        <span className="ml-3 text-base font-normal text-sky-400">Aviation Track</span>
      </motion.h2>
